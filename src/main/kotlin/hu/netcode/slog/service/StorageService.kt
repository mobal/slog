...
class StorageService(
    ...
    fun putObject(bucketName: String, key: String, data: String, mime: String): ResponseEntity<Unit> {
        return when (val result = decode(data)) {
            is Result.Success -> putObject(bucketName, key, result.value, mime)
            is Result.Failure -> putObject(bucketName, key, data.toByteArray(), mime)
        }
    }
        ...
        when (val putObjectResult = s3Service.putObject(bucketName, key, ByteArrayInputStream(bytes), mime)) {
            is Result.Success -> {
                ...
                when (val getUrlResult = s3Service.getUrl(bucketName, key)) {
                    is Result.Success -> return ResponseEntity.created(getUrlResult.value).build()
                    is Result.Failure -> throw getUrlResult.error
                }
            }
            is Result.Failure -> throw putObjectResult.error
        }
    }
    ...
    @Throws(exceptionClasses = [AmazonClientException::class])
    fun getObjectMetaData(bucketName: String, key: String): ObjectMetadataDto {
        when (val result = s3Service.getObjectMetaData(bucketName, key)) {
            is Result.Success -> return result.value.toObjectMetaDataDto()
            is Result.Failure -> throw result.error
        }
    }
    ...
    fun listBuckets(): List<BucketDto> {
        when (val result = s3Service.listBuckets()) {
            is Result.Success -> return result.value.map { it.toBucketDto() }
            is Result.Failure -> throw result.error
        }
    }
    ...
    fun listObjects(bucketName: String): List<S3ObjectDto> {
        when (val result = s3Service.listObjects(bucketName)) {
            is Result.Success -> return result.value.objectSummaries.map { it.toS3ObjectDto() }
            is Result.Failure -> throw result.error
        }
    }
}
